...
export default function UseCasesOverviewPage() {
  ...
  const useCases = isAr
    ? [
        ...
        { label: "علامات التجارة الإلكترونية", href: "/use-cases/e-commerce-brands" },
        { label: "المهنيون العقاريون", href: "/use-cases/real-estate-professionals" },
        { label: "المؤسسات التعليمية", href: "/use-cases/educational-institutions" },
        { label: "المؤثرون", href: "/use-cases/social-media-brands-influencers" },
      ]
    : [
        { label: "High-Ticket Service Providers", href: "/use-cases/high-ticket-service-providers" },
        { label: "E-commerce Brands", href: "/use-cases/e-commerce-brands" },
        { label: "Real Estate Professionals", href: "/use-cases/real-estate-professionals" },
        { label: "Educational Institutions", href: "/use-cases/educational-institutions" },
      ...
      <main>
        <section className="px-3 pt-10 sm:px-4 md:pt-16">
          <div className="mx-auto max-w-6xl rounded-[28px] bg-gradient-to-br from-cyan-50 via-white to-violet-50 px-4 pb-16 pt-8 text-center shadow-[inset_0_1px_0_rgba(255,255,255,0.7)] sm:px-6 md:pb-20 md:pt-12">
            <p className="inline-flex items-center gap-2 rounded-full border border-emerald-100 bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700">
              ...
              {isAr ? "نظرة عامة على حالات الاستخدام" : "Use Cases Overview"}
            </p>
            <h1 className="mt-5 font-display text-[1.9rem] font-bold leading-[1.2] text-ink sm:text-4xl md:text-5xl">
              {isAr ? "محادثات مبيعات بشرية الذكاء عبر مختلف القطاعات" : "Human-Like AI Sales Across Industries"}
            </h1>
            <p className="mx-auto mt-5 max-w-3xl text-base leading-relaxed text-ink-muted md:text-lg">
              {isAr
                ? "لكل نشاط تجاري تحدياته الخاصة، لكن الفرصة واحدة: تحويل المحادثات اليومية إلى عائد فعلي قابل للقياس."
                : "Every business has different sales challenges, but all share the same opportunity: turning daily conversations into measurable revenue."}
            </p>
            <p className="mx-auto mt-3 max-w-3xl text-base leading-relaxed text-ink-muted md:text-lg">
              {isAr
                ? "يتكيف Easy Reply مع طبيعة قطاعك، ويقدم محادثات بيع ذكية وعالية التحويل عبر واتساب وإنستغرام وفيسبوك ماسنجر ودردشة الموقع."
                : "Easy Reply adapts to your industry and delivers high-converting, human-like conversations across WhatsApp, Instagram, Facebook Messenger, and web chat."}
            </p>
          </div>
        </section>
        ...
        <section className="bg-white px-3 py-14 sm:px-4 md:py-20">
          <div className="mx-auto max-w-4xl text-center">
            <p className="inline-flex items-center gap-2 rounded-full border border-emerald-100 bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700">
              <span className="h-1.5 w-1.5 rounded-full bg-accent" aria-hidden />
              {isAr ? "حالات استخدام تفصيلية" : "Detailed Use Cases"}
            </p>
            <h2 className="mt-5 font-display text-3xl font-bold leading-tight text-ink md:text-5xl">
              {isAr ? "حلول مخصصة تصنع تحويلًا حقيقيًا" : "Tailored Solutions That Convert"}
            </h2>
            <p className="mx-auto mt-5 max-w-3xl text-base leading-relaxed text-ink-muted md:text-lg">
              {isAr
                ? "اكتشف كيف تستخدم الشركات في قطاعات مختلفة Easy Reply لأتمتة المحادثات، والتفاعل الطبيعي مع العملاء المحتملين، ورفع معدلات التحويل بشكل واضح."
                : "See how teams across industries use Easy Reply to automate conversations, engage prospects naturally, and increase conversion rates."}
            </p>
          </div>

          <div className="mx-auto mt-10 max-w-6xl rounded-[22px] border border-cyan-100/80 bg-gradient-to-r from-cyan-100 via-fuchsia-50/40 to-lime-100/70 p-5 shadow-[0_10px_28px_-20px_rgba(15,23,42,0.45)] sm:p-7">
            <ul className="flex flex-wrap items-center justify-center gap-3">
              {useCases.map((item) => (
                <li key={item.href}>
                  <a
                    href={item.href}
                    className="inline-flex items-center gap-2 rounded-xl border border-gray-100 bg-white px-4 py-2 text-sm font-medium text-ink shadow-[0_6px_16px_-12px_rgba(15,23,42,0.65)] transition hover:-translate-y-0.5 hover:text-accent"
                  >
                    <span className="inline-block h-2.5 w-2.5 rounded-full bg-gray-800" aria-hidden />
                    {item.label}
                  </a>
                </li>
              ))}
